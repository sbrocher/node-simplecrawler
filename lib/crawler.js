...
Crawler.prototype.fetchQueueItem = function(queueItem) {
	...
	process.nextTick(function() {
		...
		// node.js clientRequest.setTimeout won't timeout if connection is never established: 
		// it only seems to timeout on inactivity after connection has been established
		// so we use plain old timeout instead
...
Crawler.prototype.handleResponse = function(queueItem,response,timeCommenced) {
	...
	// We've got a not-modified response back
		...
		if(!queueItem.fetched) {
			queueItem.fetched = true;
			queueItem.status = "failed";		// todo: define new status instead
...
Crawler.prototype.crawl = function() {
	...
	if (crawler._openRequests >= crawler.maxConcurrency || crawler._openListeners > 0) {
		return;
	}
	...
	crawler.queue.oldestUnfetchedItem(function(err,queueItem) {
		if (queueItem) {
			...
			if (crawler._openRequests <= 0 && crawler._openListeners <= 0) {
