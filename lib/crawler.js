...
Crawler.prototype.fetchQueueItem = function(queueItem) {
	...
	process.nextTick(function() {
		...
		clientRequest.setTimeout(crawler.timeout, function() {
			...
			// when a redirect, 404, etc. is found, timeout here may still get executed - only emit fetchtimeout event if status was not set, or set to spooled or headers
			if(!queueItem.status || queueItem.status == "spooled" || queueItem.status == "headers") {
...
Crawler.prototype.handleResponse = function(queueItem,response,timeCommenced) {
	...
	// We've got a not-modified response back
		...
		// shouldn't we do this here and set the queueItem.fetched & queueItem.status too?
		// crawler._openRequests --;
