...
Crawler.prototype.processURL = function(URL,context) {
	...
	if (!URL || !URL.replace(/\s+/ig,"").length) return false;
...
Crawler.prototype.queueLinkedItems = function(resourceData,queueItem,decompressed) {
	...
	resources.forEach(function(url){ crawler.queueURL(url,queueItem,false); });
...
/*
	...
	Examples
		...
		crawler.queueURL("http://www.google.com/",queueItem,false);

	Returns a boolean value indicating whether the URL was successfully queued
	or not.

*/
Crawler.prototype.queueURL = function(url,queueItem,skipDomainCheck) {
	var crawler = this;
	var parsedURL =
	...
	if (skipDomainCheck || crawler.domainValid(parsedURL.host)) {
		crawler.queue.add(
			parsedURL.protocol,
			parsedURL.host,
			parsedURL.port,
...
Crawler.prototype.fetchQueueItem = function(queueItem) {
	...
	process.nextTick(function() {
		...
		clientRequest.setTimeout(crawler.timeout, function() {
			...
			// When a resource is being downloaded (or even when redirected, not found, etc), this may still be 
			// triggered if the socket was iddle for crawler.timeout ms (possibly also generating a race condition)
			// In those cases, queueItem.fetch would be true, so
			// only emit fetchtimeout event if queueItem.fetch is falsy 
			// (and if so also decrement crawler._openRequests and flag item as fetched to allow crawl completion)
...
Crawler.prototype.handleResponse = function(queueItem,response,timeCommenced) {
	...
	function receiveData(chunk, ended) {
		...
		if(ended && (!response.complete && responseLengthReceived < responseLength)) {
			stateData.sentIncorrectSize = true;
			console.error("Received less data than expected!", ended, response.complete, responseLengthReceived, responseLength, queueItem.url);
		}

		// if ended is called on socket, they don't timeout, so we have to handle that
		if ((ended || responseLengthReceived >= responseLength || response.complete) &&
			!dataReceived) {
	...
	// We've got a not-modified response back
		...
		// shouldn't we set the queueItem.fetched, queueItem.status, and decrement openRequests here too?
		// for now, this is taken care of by timeout/abort instead
		// crawler._openRequests --;
	...
	// If we should queue a redirect
		...
		crawler.queueURL(parsedURL,queueItem,false);
...
Crawler.prototype.crawl = function() {
	...
	if (crawler._openRequests > crawler.maxConcurrency || crawler._openListeners > 0) return;
